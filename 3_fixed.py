...
# NEW: Thread-safe SharePoint data loader
class SharePointDataWorker(QObject):
    """Worker for loading SharePoint data in background thread"""
    finished = pyqtSignal(object)  # DataFrame
    error = pyqtSignal(str)
    progress = pyqtSignal(str)  # Status message

    def __init__(self, lob_list):
        super().__init__()
        self.lob_list = lob_list
    ...
    def run(self):
        """Load SharePoint data with timeout handling"""
        try:
            if self._is_cancelled:
                return

            self.progress.emit("Initializing SharePoint connection...")
            
            # Create sharepoint session with timeout
            cred = HttpNtlmAuth(SID, "")
            site = Site(SITE_URL, auth=cred, verify_ssl=True, timeout=30)  # 30 second timeout

            if self._is_cancelled:
                return

            self.progress.emit("Fetching application data...")
            ...
            df = df_all[df_all['LOB'].isin(self.lob_list)]
...
# NEW: Thread-safe SharePoint update worker
class SharePointUpdateWorker(QObject):
    """Worker for updating SharePoint data in background thread"""
    finished = pyqtSignal()
    error = pyqtSignal(str)
    progress = pyqtSignal(str)

    def __init__(self, data_list, operation):
        super().__init__()
    ...
    def run(self):
        """Update SharePoint data with timeout handling"""
        try:
            if self._is_cancelled:
                return

            self.progress.emit("Connecting to SharePoint...")
            
            cred = HttpNtlmAuth(SID, "")
            site = Site(SITE_URL, auth=cred, verify_ssl=False, timeout=30)  # 30 second timeout

            if self._is_cancelled:
                return

            self.progress.emit("Updating data...")
            
            # Fetch data from SharePoint list
            sp_list = site.list(SHAREPOINT_LIST)
            sp_list.UpdateListItems(data=self.data_list, kind=self.operation)
            ...
            if not self._is_cancelled:
                self.error.emit(f"Failed to update data: {str(e)}")


# NEW: Thread-safe phonebook API worker
class PhonebookWorker(QObject):
    """Worker for phonebook API calls in background thread"""
    finished = pyqtSignal(str)  # Full name
    error = pyqtSignal(str)
    ...
    def run(self):
        """Fetch user name from phonebook API with timeout"""
        try:
            if self._is_cancelled or len(self.sid.strip()) != 7:
                return

            # Note: awmpy import moved here to avoid blocking main thread
            import awmpy
            data = awmpy.get_phonebook_data(self.sid)
            
            if not self._is_cancelled:
                self.finished.emit(data['nameFull'])
                
        except Exception as e:
            if not self._is_cancelled:
                self.error.emit(f"Failed to fetch user data: {str(e)}")
...
class AccessControlDialog(QDialog):
    def __init__(self, username, lob, parent=None):
        ...
        self.threads = []  # Keep track of QThread objects
        ...
        self.refresh_data_threaded()

    def show_loading_dialog(self, message="Processing..."):
        """Show an indefinite progress dialog with custom message"""
        ...
        # Connect cancel button if needed
    ...
    def refresh_data_threaded(self):
        """Refresh data using background thread - NON-BLOCKING"""
        self.show_loading_dialog("Loading application data...")
        
        # Create thread and worker
        thread = QThread()
        worker = SharePointDataWorker(self.lob)
        ...
        worker.progress.connect(self.update_progress_message)
    ...
    def on_data_error(self, error_msg):
        ...
        QMessageBox.warning(self, "Data Load Failed", 
                          f"Failed to load solution list: {error_msg}\nPlease retry after some time.",
    ...
    def refresh_data(self):
        """Legacy method - now calls threaded version"""
        self.refresh_data_threaded()
        return True
    ...
    def fetch_user_name_threaded(self, sid):
        """Fetch user name using background thread - NON-BLOCKING"""
        if len(sid.strip()) != 7:
            return
            
        # Create thread and worker
        thread = QThread()
        worker = PhonebookWorker(sid)
        ...
        # Connect signals
        thread.started.connect(worker.run)
        worker.finished.connect(self.on_user_name_fetched)
        worker.error.connect(self.on_user_name_error)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        worker.error.connect(thread.quit)
        worker.error.connect(worker.deleteLater)
    ...
    def on_user_name_fetched(self, full_name):
        """Handle successful user name fetch"""
        if hasattr(self, 'add_app_fields') and 'Developed_By' in self.add_app_fields:
            self.add_app_fields['Developed_By'].setText(full_name)

    def on_user_name_error(self, error_msg):
        """Handle user name fetch error"""
        if hasattr(self, 'add_app_fields') and 'Developed_By' in self.add_app_fields:
            self.add_app_fields['Developed_By'].clear()

    def fetch_user_name(self, sid):
        """Legacy method - now calls threaded version"""
        self.fetch_user_name_threaded(sid)
    ...
    def update_app_list(self):
        ...
        if hasattr(self, 'df') and not self.df.empty:
            for _, row in self.df.iterrows():
                ...
                tile_widget = AppTileWidget(row['Solution_Name'], row['Description'])
    ...
    def save_application(self):
        """Save application using background thread - NON-BLOCKING"""
        # Validate all fields
        all_valid = True
        ...
        # Show loading dialog
        self.show_loading_dialog("Saving application data...")
        ...
        if is_update_mode:
            ...
            if hasattr(self, 'df') and not self.df.empty:
                ...
                self.update_sharepoint_threaded([data_dictionary], 'Update', is_update_mode, app_name)
        else:
            self.update_sharepoint_threaded([new_data], 'New', is_update_mode, new_data['Solution_Name'])

    def update_sharepoint_threaded(self, data_list, operation, is_update_mode, app_name):
        """Update SharePoint using background thread - NON-BLOCKING"""
        # Create thread and worker
        thread = QThread()
        worker = SharePointUpdateWorker(data_list, operation)
        ...
        # Connect signals
        thread.started.connect(worker.run)
        worker.finished.connect(lambda: self.on_sharepoint_update_success(is_update_mode, app_name))
        worker.error.connect(lambda err: self.on_sharepoint_update_error(err, app_name))
        worker.progress.connect(self.update_progress_message)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        worker.error.connect(thread.quit)
    ...
    def on_sharepoint_update_success(self, is_update_mode, app_name):
        """Handle successful SharePoint update"""
        if self.progress_dialog:
            self.progress_dialog.close()
            self.progress_dialog = None
        ...
        # Log action
        try:
            action_text = f'Updated details for app {app_name}' if is_update_mode else f'New App Registration done: {app_name}'
            pslv_action_entry([{'SID': user_main, 'action': action_text}])
        except:
    ...
    def on_sharepoint_update_error(self, error_msg, app_name):
        """Handle SharePoint update error"""
        if self.progress_dialog:
            self.progress_dialog.close()
            self.progress_dialog = None
            
        QMessageBox.warning(self, "Save Failed",
                          f"Failed to save application [{app_name}]: {error_msg}",
                          QMessageBox.StandardButton.Ok)
    ...
    def update_sharepoint_db(self, dictionary_as_list, operation):
        """Legacy method - now calls threaded version"""
        # This method is kept for compatibility but operations are now threaded
        pass
    ...
    def setup_user_management(self, users_container):
        ...
        add_label = QLabel("Enter user SIDs (one per line):")
        ...
        # Connect app selection to show users
        self.app_list.itemSelectionChanged.connect(self.show_application_users)

    def show_application_users(self):
        """Show users for selected application"""
        current_item = self.app_list.currentItem()
        if not current_item:
            return
    ...
    def remove_selected_users(self):
        """
        Remove selected users using background thread - NON-BLOCKING
        """
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self.show_loading_dialog("Removing user access...")
            ...
            if hasattr(self, 'df') and not self.df.empty:
                ...
                # Update SharePoint in background
                data_dictionary = self.df.iloc[app_idx].to_dict()
                self.update_user_access_threaded([data_dictionary], 'Update', users_to_remove, app_name, 'remove')

    def add_multiple_users(self):
        """
        Add multiple users using background thread - NON-BLOCKING
        """
        if not self.app_list.currentItem():
            QMessageBox.warning(self, "No Application Selected",
                                "Please select an application first.",
        ...
        # Show progress dialog for verification
        self.show_loading_dialog("Verifying user IDs...")

        # Create worker thread for verification
        self.thread = QThread()
    ...
    def update_user_access_threaded(self, data_list, operation, users_list, app_name, action_type):
        """Update user access using background thread"""
        # Create thread and worker
        thread = QThread()
        worker = SharePointUpdateWorker(data_list, operation)
        ...
        # Connect signals
        thread.started.connect(worker.run)
        worker.finished.connect(lambda: self.on_user_access_update_success(users_list, app_name, action_type))
        worker.error.connect(lambda err: self.on_user_access_update_error(err, users_list, app_name, action_type))
        worker.progress.connect(self.update_progress_message)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        worker.error.connect(thread.quit)
    ...
    def on_user_access_update_success(self, users_list, app_name, action_type):
        """Handle successful user access update"""
        if self.progress_dialog:
            self.progress_dialog.close()
            self.progress_dialog = None
        ...
        # Log action
        try:
            action_text = f'Removed users {users_list} from {app_name}' if action_type == 'remove' else f'Added users {users_list} to {app_name}'
            pslv_action_entry([{'SID': user_main, 'action': action_text}])
        except:
            pass  # Don't fail if logging fails

        # Update UI
        self.show_application_users()
        
        # Show success message
        action_word = "removed" if action_type == 'remove' else "added"
        self.show_success_message(f"Successfully {action_word} {len(users_list)} user(s)")

    def on_user_access_update_error(self, error_msg, users_list, app_name, action_type):
        """Handle user access update error"""
        if self.progress_dialog:
            self.progress_dialog.close()
            self.progress_dialog = None

        # Revert DataFrame changes on error
        if hasattr(self, 'df') and not self.df.empty:
            app_idx = self.df[self.df['Solution_Name'] == app_name].index[0]
            # Revert the changes (this is a simplified revert - in production you'd want better error recovery)
            self.refresh_data_threaded()  # Refresh from server to get clean state

        action_word = "remove" if action_type == 'remove' else "add"
        QMessageBox.warning(self, "Update Failed",
                          f"Failed to {action_word} users: {error_msg}",
                          QMessageBox.StandardButton.Ok)
    ...
    def handle_verification_complete(self, verification_results):
        ...
        if valid_ids:
            ...
            if hasattr(self, 'df') and not self.df.empty:
                ...
                # Update SharePoint in background
                data_dictionary = self.df.iloc[app_idx].to_dict()
                self.update_user_access_threaded([data_dictionary], 'Update', valid_ids, app_name, 'add')
